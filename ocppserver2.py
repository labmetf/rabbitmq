...
class ChargePoint(cp):
    ...
    def on_metervalues(self, connector_id, meter_value, **kwargs):
        val = meter_value
        value = val[0]['sampled_value']          
        date_time_obj = datetime.strptime(val[0]['timestamp'], '%Y-%m-%dT%H:%M:%SZ')
        con_id = connector_id
        
        mycursor.execute("SELECT transaction_pk FROM transaction_start order by transaction_pk desc limit 1")
        transaction_id = mycursor.fetchone()[0] 

        for i in value :
            insert = 'connector_pk,transaction_pk,value_timestamp,'
            nilai = [con_id,transaction_id,date_time_obj]
            for val in i :
                insert = insert+val+","
                nilai.append(i[val])
            insert2 = insert[:-1]
            #sql = a+insert2+c+(3*"%s,"+len(i)*"%s,")[:-1]+")"
            sql = "INSERT INTO connector_meter_value ("+insert2+") VALUES ("+(3*"%s,"+len(i)*"%s,")[:-1]+")"
            val = tuple(nilai)
            #print(sql)
            #print(val)
        ...
        print(f'transaction_id : {transaction_id} value : {meter_value} .')
        return call_result.MeterValuesPayload()
  
    @on(Action.StartTransaction)
    def on_starttransaction(self, connector_id, id_tag, meter_start, timestamp, **kwargs):
        sql = "INSERT INTO transaction_start (connector_pk,start_timestamp,start_value) VALUES (%s,%s,%s)"
        val = (connector_id,datetime.strptime(timestamp, '%Y-%m-%dT%H:%M:%SZ'),meter_start)
        mycursor.execute(sql,val)
        mydb.commit()
    ...
    def on_status_notitication(self, connector_id, error_code, status, **kwargs):
        sql = "INSERT INTO connector_status (connector_pk,status) VALUES (%s,%s)"
        val = (connector_id,status)
        mycursor.execute(sql,val)
        mydb.commit()
    ...
    def on_stoptransaction(self, id_tag, meter_stop, timestamp, transaction_id, **kwargs):
        sql = "INSERT INTO transaction_stop (transaction_pk,stop_timestamp,stop_value) VALUES (%s,%s,%s)"
        val = (transaction_id,datetime.strptime(timestamp, '%Y-%m-%dT%H:%M:%SZ'),meter_stop)
        mycursor.execute(sql,val)
        mydb.commit()
